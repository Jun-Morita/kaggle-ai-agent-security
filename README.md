...
The 2026-08-05 evaluator / leaderboard refresh is a new scoring regime. Old and new public scores are not directly comparable. Historical best `v123_v121_exact_rerun=91.890` remains useful pre-refresh evidence, but the current confirmed post-refresh best is `v148_replaysafe_postrefresh_push=88.245` (2026-08-17), unbeaten by all subsequent throughput, compound-action, and CONFUSED_DEPUTY probes through `v158`.

A refreshed public leaderboard snapshot on 2026-08-21 had `3822` valid-scored teams. Our team is rank `619` at score `88.245`; the top-5% public Silver boundary is rank `192` at score `91.575`, and the top-10% boundary is rank `383` at `89.415`. The active gap to public Silver is `+3.330`, roughly equivalent to `37` more successful single-post `EXFILTRATION` findings at about `0.09` points per firing candidate.
...
- `v150_replaysafe_099=76.815`, `v152_fsdelete_isolated=59.190`, `v154_probehops1=77.580`, and `v155_slowmultipost2=81.365` (all 2026-08-18/19) each regressed below `v148=88.245`; convergent evidence that `v148`'s exact configuration is a local optimum and that compound multi-action-per-message shapes (`v152`, `v155`) carry a real compliance cost. `v151_threearm_rsf985=87.885` confirms two independently-positive levers (`v147`'s three-arm classifier, `v148`'s `REPLAY_SAFE_FRAC=0.985`) do not stack additively.
...
The active baseline is fundamentally limited by successful single-post candidate count. Under the 2026-08-21 refreshed snapshot (`3822` teams), the top-5% boundary is `91.575` at rank `192`, which is `+3.330` above the confirmed best `v148=88.245` -- roughly `37` more successful single-post `EXFILTRATION` findings at about `0.09` points each. A safe public Silver buffer still likely requires several additional points because the leaderboard is moving.
...
- The 2026-08-21 refreshed public leaderboard snapshot has `3822` valid-scored teams, top-5% rank `192`, and top-5% score `91.575`; `v148=88.245` is the current confirmed post-refresh best and remains outside public Silver by `+3.330`.
