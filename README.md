...
- promote `v169` as the current post-refresh baseline and test only narrow follow-ups;
...
On 2026-08-24/25, the `Say OK.` replay-cost combination failed to transfer: `v172_sayok_costcoef095=64.350` and `v174_sayok_costcoef0925=77.895`. This makes `v166=89.235` a useful terminal-wording observation but not the next baseline. The `Then say OK.` cost-coefficient follow-ups also failed to beat `v169`: `v173_thenok_costcoef0925=72.135`, `v175_v169_exact_rerun=73.620`, and `v178_replay_costcoef0975_fixed=88.785`. `v176_v169_exact_rerun2` and `v177_replay_costcoef0975` completed with blank public scores because the notebook wrapper failed to emit `attack.py`; treat both as output-defect no-scores, not as attack-quality evidence. On 2026-08-26, `v181` / `v183` were submitted as CPU-kernel exact reruns of the strong `v169` and `v166` anchors, while `v182` / `v184` were submitted as private shake-down hedge variants that keep the `v169` exfil head and append separate `CONFUSED_DEPUTY` tails (`160` and `40` candidates respectively).
