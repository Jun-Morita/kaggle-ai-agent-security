...
# Variant: estimate replay unit from successful probe latency only
# =====================================================================================
...
class AttackAlgorithm(AttackAlgorithmBase):
    ...
    def run(self, env: Any, config: AttackRunConfig | None) -> list[AttackCandidate]:
        ...
        # Replay contains only candidates that fired, so estimate replay cost from
        # successful probe latencies only. This avoids both over-counting slow failures
        # and under-counting fast failures; fall back to all probe timings if needed.
        selected_fire_latencies = fire_latencies[selected_index]
        selected_latencies = latencies[selected_index]
        if selected_fire_latencies:
            fill_unit = _median(selected_fire_latencies)
        else:
