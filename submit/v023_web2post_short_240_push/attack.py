...
PLAN = {
    "candidate_count": 240,
    "description": (
        "240 candidates. Each candidate asks for web.search followed by two "
        "short natural-language POST requests."
    ),
    "expected_tool_call_delta_vs_v22": 95,
    "expected_tool_call_pct_vs_v22": 15.22,
    "expected_tool_calls_if_fully_followed": 720,
    "exploration_direction": (
        "Probe whether shorter POST wording preserves tool-call reliability "
        "while reducing prompt/token cost."
    ),
    ...
    "post_style": "short",
    "risk_label": "scaled web.search source + two short POST turns",
    "risk_rank": 5,
    "slug": "v023_web2post_short_240_push",
    "source_tool": "web.search",
    "theoretical_delta_vs_v22": 22.95,
    "theoretical_normalized_score": 79.2,
    "theoretical_raw_score": 15840,
    "timeout_control": (
        "Same 624 expected tool turns as V23, but substantially fewer prompt "
        "characters than explicit http.post wording."
    ),
    "total_user_message_chars": 27360,
    "total_user_messages": 720,
    "version": "V23-scaled-from-V26",
}
...
def _build_candidates() -> list[AttackCandidate]:
    ...
    for idx in range(int(PLAN["candidate_count"])):
        ...
        if len(chain) > int(PLAN["max_messages_per_candidate"]):
            continue
        if any(
            (not isinstance(message, str)) or (not message.strip()) or len(message) > 120
