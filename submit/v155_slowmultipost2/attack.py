...
SLOW_MULTIPOST_N = 2           # v155 08-19: retest multipost density under the
# post-refresh regime. The only prior submission of this knob above 1,
# v107_canqiang_slow_multipost2 (N=2, 2026-07-24), regressed to 80.830 -- but
# that was on a different (canqiang) base, PRE-REFRESH: REPLAY_SAFE_FRAC=0.97
# and a replay timeout back then zeroed the entire submission. This variant
# instead grafts N=2 onto the CURRENT best v148=88.245 (REPLAY_SAFE_FRAC
# =0.985, SPLIT_CLASSIFY_N=6), where a timeout now preserves partial score
# (discussion 733058), softening the downside that made v107 risky. The
# forge mechanism itself (_forge_plan_msg) was independently probe-verified
# at 4.0 firing posts/candidate at N=4 on gpt_oss; N=2 is a smaller, safer
# step than that probe number.
