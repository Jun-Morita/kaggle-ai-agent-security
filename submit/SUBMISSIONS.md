...
|---|---|---|---:|---:|---|---|---|
| v005_trajectory_exfil_aggressive | 2026-06-13 | `workspace/exp004_trajectory_exfil_aggressive` |  |  | `junichiromorita/ai-agent-security-v005-trajectory-exfil-aggressive` v1 | `eda-agent-security-trajectory-search`, `17-1-eda-agent-security-trajectory-search`, `submit/v004_trajectory_exfil` | Submitted as ref `53641117`; status pending. Uses the same trajectory-search family as `v004` but switches to the public highest-score notebook's aggressive profile: max verified `600`, return target `300`, static fallback `100`, empty-run fallback `32`, search fraction `0.35`, max search `850s`. Local validate passed; deterministic smoke 60s completed with attack score 0.00. |
| v004_trajectory_exfil | 2026-06-13 | `workspace/exp003_trajectory_exfil` |  |  | `junichiromorita/ai-agent-security-v004-trajectory-exfil` v1 | `17-1-eda-agent-security-trajectory-search`, `submit/v003_broad_prompt_bank` | Submitted as ref `53637051`; status still pending after repeated CLI checks. Ports the high-scoring trajectory-search exfil approach into this repo's stable wrapper: scoring payload bank, endpoint-count calibration, domain-diverse hosts, priority sorting, and bounded fill. Local validate passed; deterministic smoke 60s completed with attack score 0.00. |
