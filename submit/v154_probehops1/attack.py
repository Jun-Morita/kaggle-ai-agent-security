...
PROBE_HOPS = 1                 # v154 08-19: 1-hop probe for ~1.5-2x faster fill
REPLAY_COST_COEF = 1.8        # scale under-counted 1-hop elapsed back up to the
# estimated hops=8 replay cost, matching the pairing proven safe-to-run in
# v111_probehops1_coef18 (2026-07-26). v111 itself completed with an
# "incorrect format" / no-score result, but that was PRE-REFRESH: a replay
# timeout back then zeroed the entire submission. Post-2026-08-05 (discussion
# 733058), a replay timeout now PRESERVES the score accumulated before it, so
# the downside of this throughput push is much softer than when v111 failed.
# Base is v148=88.245 (the current confirmed best); only PROBE_HOPS and
# REPLAY_COST_COEF change from it -- REPLAY_SAFE_FRAC stays at 0.985,
# SPLIT_CLASSIFY_N stays at 6.
