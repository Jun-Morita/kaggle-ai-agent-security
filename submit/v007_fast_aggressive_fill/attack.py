...
MAX_VERIFIED_CANDIDATES = min(120, int(SDK_MAX_REPLAY_FINDINGS))
RETURN_TARGET_CANDIDATES = min(300, int(SDK_MAX_REPLAY_FINDINGS))
STATIC_FALLBACK_CANDIDATES = RETURN_TARGET_CANDIDATES
EMPTY_RUN_FALLBACK_CANDIDATES = RETURN_TARGET_CANDIDATES
SEARCH_FRACTION = 0.18
MAX_SEARCH_S = 360.0
ENDPOINT_COUNTS_TO_CALIBRATE = (1, 2)
ENDPOINTS_PER_MSG = 2
MAX_MSG_CHARS = 1900
DEFAULT_MAX_HOPS = 4
...
class AttackAlgorithm(AttackAlgorithmBase):
    """Fast aggressive-fill trajectory attacker with private-safe exfil payloads."""
    ...
    def _static_candidates(self, n: int, family_prefix: str) -> list[AttackCandidate]:
        ...
        endpoint_count = min(2, ENDPOINTS_PER_MSG, DEFAULT_MAX_HOPS)
    ...
    def run(self, env, config: AttackRunConfig) -> list[AttackCandidate]:
        ...
        calibration_end = started + search_s * 0.25
        mutation_end = started + search_s * 0.34
        mass_end = started + search_s * 0.50
        single_end = started + search_s * 0.58
        deputy_end = started + search_s * 0.64
        ...
        if not good_combos:
            default_endpoint = min(2, max_hops)
            good_combos = [
                {
                    "score": 0.0,
                    "post_count": 0,
        ...
        for base_msg in list(dict.fromkeys(successful_single_messages))[:4]:
        ...
        combo_cycle = itertools.cycle(good_combos[: max(1, min(6, len(good_combos)))])
